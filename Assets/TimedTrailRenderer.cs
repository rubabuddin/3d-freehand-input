...
public class TimedTrailRenderer : MonoBehaviour
{
   ...
   void Start()
   {
		meshSaver = GameObject.Find ("ForSavingMesh");
      lastPosition = transform.position;
      o = new GameObject("Trail");
		if (meshSaver)
						o.transform.parent = meshSaver.transform;
		else
   ...
   void Update ()
   {
	...
	//	Debug.Log (theDistance);
		if (theDistance == 0) {
			theDistance = (lastPosition - transform.parent.position).magnitude;
				}
      if(emit)
      {
